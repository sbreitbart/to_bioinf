...
```{r}
# fit most models
fit_basic_mod <- function(data, response, urb_predictor) {
  model <- glmmTMB(response ~ urb_predictor,
                   data)
  print(model)
...
# fit private allele & pi models (includes random effect for number of individuals per population)
fit_glmer <- function(data, response, urb_predictor, family) {
  model <- glmmTMB(response ~ urb_predictor + (1|num_indv), 
                   data, 
                   family = family)
  print(model)
  print(performance::check_model(model))
...
### city_dist
priv_dist <- fit_glmer(data = diversity_all_sites,
                           response = diversity_all_sites$private,
urb_predictor = diversity_all_sites$City_dist,
family = "nbinom2")

### Urb_score
priv_usc <- fit_glmer(diversity_all_sites,
                          response = diversity_all_sites$private,
urb_predictor = diversity_all_sites$urb_score,
family = "nbinom2")
```
...
```{r}
### City_dist
Het_exp_dist <- fit_basic_mod(diversity_all_sites,                            response = diversity_all_sites$exp_het,
                            urb_predictor = diversity_all_sites$City_dist)

### Urb_score
Het_exp_usc <- fit_basic_mod(diversity_all_sites,                            response = diversity_all_sites$exp_het,
                           urb_predictor = diversity_all_sites$urb_score)
...
```{r}
### City_dist
Het_obs_dist <- fit_basic_mod(diversity_all_sites,                            response = diversity_all_sites$obs_het,
                            urb_predictor = diversity_all_sites$City_dist)

### Urb_score
Het_obs_usc <- fit_basic_mod(diversity_all_sites,                            response = diversity_all_sites$obs_het,
                            urb_predictor = diversity_all_sites$urb_score)
...
```{r}
### City_dist
Hom_exp_dist <- fit_basic_mod(diversity_all_sites,
                              response = diversity_all_sites$exp_hom,
                            urb_predictor = diversity_all_sites$City_dist)

### Urb_score
Hom_exp_usc <- fit_basic_mod(diversity_all_sites,
                             response = diversity_all_sites$exp_hom,
                             urb_predictor = diversity_all_sites$urb_score)
...
```{r}
### City_dist
Hom_obs_dist <- fit_basic_mod(diversity_all_sites,
                            response = diversity_all_sites$obs_hom,
                            urb_predictor = diversity_all_sites$City_dist)

### Urb_score
Hom_obs_usc <- fit_basic_mod(diversity_all_sites,
                            response = diversity_all_sites$obs_hom,
                            urb_predictor = diversity_all_sites$urb_score)
...
```{r}
### City_dist
pi_dist <- fit_glmer(diversity_all_sites,
                          response = diversity_all_sites$pi,
urb_predictor = diversity_all_sites$City_dist,
family = "gaussian")

### urb_score
pi_usc <- fit_glmer(diversity_all_sites,
                          response = diversity_all_sites$pi,
urb_predictor = diversity_all_sites$urb_score,
family = "gaussian")
```
...
create_anova_df <- lapply(mod_list, function(model) {
  ...
  Rsq <- MuMIn::r.squaredGLMM(priv_dist)[1] %>%
  round(digits = 3) %>%
  format(nsmall = 3)
