...
vcf_larger_pops <- read.vcfR(
  here::here("./filtered_vcfs/pops_MORE_THAN_1indiv_per_pop_mmaf0.05_variant_sites.recode.vcf"),
  verbose = FALSE)
...
pops_larger <- read.csv(here("./clean_data/pop_map3_MORE_THAN_1indiv_per_pop.csv")) %>%
...
vcf_larger_pops_urb <- read.vcfR(
  here::here("./filtered_vcfs/pops_MORE_THAN_1indiv_per_pop_URBAN_mmaf0.05_variant_sites.recode.vcf"),
  verbose = FALSE)
...
vcf_larger_pops_rur <- read.vcfR(
  here::here("./filtered_vcfs/pops_MORE_THAN_1indiv_per_pop_RURAL_mmaf0.05_variant_sites.recode.vcf"),
  verbose = FALSE)
...
# this is going to be complicated... my vcf isn't the right format...
# create genotype matrix
test <- vcf2geno(vcf, 
         none = "0/0", one = c("0/1"), both = "1/1",
         na.string = ".",
         use.rownames = FALSE, 
         allowDifference = FALSE, 
         removeMonomorphic = TRUE, 
		removeNonBiallelic = TRUE, 
		changeMinor = FALSE)


# estimate FST using the "Hudson" formula
popkinsuppl::fst_hudson_k_obj <- fst_hudson_k(,
                                 formatted_genind[,1])
...
# this will only work for one hierarchical level... not multiple at once.
vcf_h <- hierfstat::read.VCF(here::here("./summary_stats/output_populations_no-IBD/mmaf0.05_R0.75/populations.snps.vcf"))
