...
# sampling site IDs and genetic sample names
coords_subsamp <- read.csv(here("./clean_data/pop_maps/pop_map3_1indiv_per_pop.csv")) %>%
...
#### Decrease resolution from 15x15 to 90x90 (6x) & reproject
##### 90x, for all individuals
```{r}
...
##### 120x, for 1 individual/site
```{r}
gta_solris_120x <- aggregate(gta_solris,
                            fact = 8,
                            # aggregation method = mode
                            fun = modal)

# check classes
unique(gta_solris_120x)

# remove old raster
rm(gta_solris)

# export as a GeoTIFF
# writeRaster(gta_solris_120x,
#             filename = here::here("./clean_data/radish/gta_solris_120x.tif"),
#             format = "GTiff")


# Reproject the raster
gta_solris_120x_reproj <- projectRaster(gta_solris_120x,
                                     crs = target_crs,
                                    # nearest neighbor
                                    method = "ngb")

# check classes
unique(gta_solris_120x_reproj)

# Check the new CRS
crs(gta_solris_120x_reproj)


# export as a GeoTIFF
# writeRaster(gta_solris_120x_reproj,
#             filename = here::here("./clean_data/radish/gta_solris_120x_reproj.tif"),
#             format = "GTiff")
```

#### Reproject the points to the same CS
##### All individuals
```{r}
...
```{r}
# Create a SpatialPointsDataFrame
# BUT FIRST, get coords_subsamp df from chunk above

your_spdf_subsamp <- SpatialPointsDataFrame(coords_subsamp, 
                                    data = coords_subsamp, 
                                    proj4string = CRS("+proj=longlat +datum=WGS84"))

# Project to UTM Zone 17
your_spdf_utm_subsamp <- spTransform(your_spdf_subsamp, target_crs)

# Check the new CRS
proj4string(your_spdf_utm_subsamp)


# overlay points onto raster
plot(gta_solris_120x_reproj)
points(your_spdf_utm_subsamp, col = "red", pch = 20)

# Convert SpatialPointsDataFrame to SpatialPoints
sp_subsamp <- SpatialPoints(your_spdf_utm_subsamp)
class(sp_subsamp)  # This will print "SpatialPoints"
...
##### 1 individual/site (120x)
```{r}
# Get the coordinates of the points
coords_subsamp <- coordinates(your_spdf_utm_subsamp)

# Calculate the convex hull
convex_hull_idx_subsamp <- chull(coords_subsamp)

# Create a SpatialPolygons object for the convex hull
convex_hull_subsamp <- SpatialPolygons(list(Polygons(list(Polygon(coords_subsamp[convex_hull_idx_subsamp, ])), ID = 1)))

# Create a buffer around the convex hull (5km buffer)
buffered_hull_subsamp <- rgeos::gBuffer(convex_hull_subsamp,
                                width = 5000)

# Crop the raster to the buffered convex hull
gta_solris_120x_reproj_cropped <- crop(gta_solris_120x_reproj,
                       as(buffered_hull_subsamp,
                          "SpatialPolygons"))

# Plot the cropped raster
plot(gta_solris_120x_reproj_cropped)

# Overlay the projected SpatialPoints on the raster
points(your_spdf_utm_subsamp, col = "red", pch = 20)

# check classes intact
unique(gta_solris_120x_reproj_cropped)

# check CS
crs(gta_solris_120x_reproj_cropped)


# export as a GeoTIFF
# writeRaster(gta_solris_120x_reproj_cropped,
#             filename = here::here("./clean_data/radish/gta_solris_120x_reproj_cropped.tif"),
#             format = "GTiff")
```
...
##### 1 individual/site (120x)
```{r}
lulc_raster_subsamp = mask(gta_solris_120x_reproj_cropped,
          buffered_hull_subsamp)

plot(lulc_raster_subsamp)

# Overlay the projected SpatialPoints on the raster
points(your_spdf_utm_subsamp, col = "red", pch = 20)

str(lulc_raster_subsamp)
res(lulc_raster_subsamp)
unique(lulc_raster_subsamp)

# export as a GeoTIFF
# writeRaster(lulc_raster_subsamp,
#             filename = here::here("./clean_data/radish/lulc_raster_subsamp.tif"),
#             format = "GTiff")
```
...
##### 1 individual/site (120x)
```{r}
lulc_integers_subsamp <- unique(lulc_raster_subsamp)
print(lulc_integers_subsamp)

# this is missing class 140 (fen), but otherwise identical to previous lulc_raster


# Reclassify these classes-----
# BUILT-UP AREA (x --> 203)-----
# 202 --> 203
lulc_raster_reclassified_subsamp <- reclassify(lulc_raster_subsamp, 
                             matrix(
                               c(202, # original class
                                 203), # new class
                               ncol = 2, 
...
# 201 --> 203
lulc_raster_reclassified_subsamp <- reclassify(lulc_raster_reclassified_subsamp, 
                             matrix(
                               c(201, # original class
                                 203), # new class
                               ncol = 2, 
...
# check number of pixels with each LULC-----
sort(table(getValues(lulc_raster_reclassified_subsamp)),
                            decreasing = TRUE)

#   203      250    170      193         93      131 
#  82419    14452  13723    8341        6604    2292 
# Built-up  Other  Water  Agriculture Forested  Swamp
...
##### 1 individual/site (120x)
```{r}
pdf(here::here("./Figures_Tables/radish/reclassified_raster_discrete_points_1indiv.pdf"),         # File name
    width = 8, height = 7, # Width and height in inches
    bg = "white",          # Background color
    paper = "A4")          # Paper size
...
# Plot the raster with discrete legend
plot(lulc_raster_reclassified_subsamp,
     legend = FALSE, 
     col = c("limegreen", "yellow", "cyan", "brown", "gray", "magenta"))
...
# Overlay the projected SpatialPoints on the raster
points(your_spdf_utm_subsamp, col = "black", pch = 21)
...
##### 1 individual/site (120x)
```{r}
lulc_raster_rat_subsamp <- raster::ratify(lulc_raster_reclassified_subsamp)

# create rat table

# unique(lulc_raster_reclassified_subsamp)
# [1]  93 131 170 193 203 250

rat_table_subsamp <- data.frame(ID = c(unique(lulc_raster_reclassified_subsamp)),
                  VALUE = c(
                    "Forested",
                    "Swamp",
                    "Water",
...
levels(lulc_raster_rat_subsamp) <- rat_table_subsamp
...
vcf_subsamp <- read.vcfR(
  here::here("./filtered_vcfs/1indiv_per_mmaf0.05_variant_sites_URBAN.recode.vcf"),
  verbose = FALSE)
...
# tidy up colnames (remove periods)
inputdata1 <- my_genind_subsamp$tab
...
### 1 individual/site (120x)
```{r}
stacked_covs_subsamp <- stack(lulc_raster_rat_subsamp)

print(stacked_covs_subsamp)
```
...
### 1 individual/site
```{r}
surface_subsamp <- conductance_surface(covariates = stacked_covs_subsamp,
                               coords = sp_subsamp,
                               directions = 8)
```
...
### 1 individual/site
```{r}
genetic_distance_matrix_subsamp2 <- genetic_distance_matrix_subsamp %>%
  as.matrix()

# this model didn't fully converge when I used the 90x OR 105x rasters (under the [1] f(x) = ... line, it says, "... switched to backtracking" for the 90x, and for the 105x, it had the backtracking line under 1 and 2).

fit_mlpe_subsamp <- radish(
  # the SOLRIS part is actually the name/covariate
  genetic_distance_matrix_subsamp2 ~ SOLRIS_Version_3_0_LAMBERT,
  data = surface_subsamp, 
  conductance_model = radish::loglinear_conductance,
  measurement_model = radish::mlpe)

# output for 120x model:
# Projected Newton-Raphson with Hager-Zhang line search
# [1] f(x) = 7.932e+03   |f(x)-fold(x)| = 0.000e+00   max|f'(x)| = 4.442e-02   |f''(x)| = -4.081e-10 
# ... switched to backtracking
# [2] f(x) = 7.933e+03   |f(x)-fold(x)| = 1.200e+00   max|f'(x)| = 2.475e-02   |f''(x)| = -2.185e+05 
# ... switched to backtracking
# [3] f(x) = 7.933e+03   |f(x)-fold(x)| = 3.909e-02   max|f'(x)| = 9.764e-04   |f''(x)| = -6.530e+00 
# [4] f(x) = 7.933e+03   |f(x)-fold(x)| = 1.988e-04   max|f'(x)| = 7.202e-04   |f''(x)| = -1.223e+02 
# [5] f(x) = 7.933e+03   |f(x)-fold(x)| = 5.920e-04   max|f'(x)| = 5.276e-04   |f''(x)| = 1.448e-02 
# Error in .local(a, b, ...) : 
...
# examine model
print(summary(fit_mlpe_subsamp))
...
## Plot fitted conductance surface
fitted_conductance_subsamp <- conductance(surface,
                                  fit_mlpe_subsamp,
                                  quantile = 0.95)

plot(
  log(fitted_conductance_subsamp[["est"]]), 
     main = "Fitted conductance surface for 1 individual/site")
...
for (i in 1:nlayers(fitted_conductance)) {
  ...
  writeRaster(fitted_conductance_subsamp[[i]],
              filename = output_path, 
              format = "GTiff")
}
```
...
pdf(here::here("./Figures_Tables/radish/conductance_surface_1indiv.pdf"),        
    width = 8, height = 7, 
    bg = "white",          
    colormodel = "cmyk" , 
    paper = "A4")         
...
# Creating a plot
plot(log(fitted_conductance_subsamp[["est"]]),
     main = "Fitted conductance surface- urban core- 1 individual/site")

# Closing the graphical device
dev.off()
```

# Export table with coefficients and p-values
## All individuals
```{r}
data.frame("Category" = c(
  ...
  flextable::flextable() %>%
  flextable::align(part = "all", j = 2:3, align = "center") %>%
  flextable::autofit() %>%
  save_as_docx(path = here::here("./Figures_Tables/radish/radish_model_output.docx"))
...
data.frame("Category" = c(
  ...
  flextable::align(part = "all", j = 2:3, align = "center") %>%
  flextable::autofit() %>%
  save_as_docx(path = here::here("./Figures_Tables/radish/radish_model_output_1indiv.docx"))
