...
diversity_variant_sites <- here::here("./populations_no-IBD_vcf-all_mainfiles/mmaf0.05_R0.75/populations.sumstats_summary.tsv") %>%
...
test1 <- glmmTMB(urb_score ~ num_indv, diversity_variant_sites)
performance::check_model(test1)
car::Anova(test1) # no
```
...
##### Distance
```{r}
fis_dist_cat <- glmmTMB(fis ~ u_r_dist + (1|num_indv),
           diversity_variant_sites %>%
             dplyr::filter(fis != 0))
...
##### Urb score
```{r}
fis_usc_cat <- glmmTMB(fis ~ u_r_usc + (1|num_indv),
           diversity_variant_sites %>%
             dplyr::filter(fis != 0))
...
##### Distance
```{r}
pi_dist_cat <- glmmTMB(pi ~ u_r_dist + (1|num_indv),
           diversity_variant_sites)
...
##### Urb score
```{r}
pi_usc_cat <- glmmTMB(pi ~ u_r_usc + (1|num_indv),
           diversity_variant_sites)
