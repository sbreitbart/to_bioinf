...
grp <- adegenet::find.clusters(my_genind,
                        ...
                        # since I've been through this and seen that the lowest BIC is for 1-4 clusters, I'm making this smaller to see the detail better
                        max.n.clust= 10, 
                        var.contrib = TRUE, 
                        scale = FALSE,
...
hulls_k3 <- plyr::ddply(group_membership_k3, "cluster", find_hull)
...
get_stamenmap(c(left = -80.5, bottom = 43.2, right = -78.5, top = 44.15),
              ...
              maptype = "terrain") %>%
  ggmap() +
  geom_point(data = group_membership_k3,
             aes(x = long,
             y = lat,
             fill = cluster,
             shape = cluster),
             size = 2,
             color = "black") +
      geom_polygon(data = hulls_k3,
                 mapping = aes_string(
                   x = "long", 
                   y = "lat", 
                   color = as.factor(hulls_k3$cluster), 
                   fill = as.factor(hulls_k3$cluster)),
                 alpha = 0.3) +
  xlab("Longitude") +
  ylab("Latitude") +
  scale_shape_manual(values = c(21,22,23))

ggsave(here::here("./Figures_Tables/dapc/k3_map.png"),
         width = 16,
         height = 11,
         units = "cm")

# violin plot of clusters vs urbanization
ggplot(group_membership_k3,
       aes(y = City_dist,
       x = cluster)) +
  geom_violin(aes(y = City_dist,
             x = cluster,
...
ggsave(here::here("./Figures_Tables/dapc/k3_dotplot.png"),
         width = 16,
         height = 11,
         units = "cm")
...
hulls_k4 <- plyr::ddply(group_membership_k4, "cluster", find_hull)
...
get_stamenmap(c(left = -80.5, bottom = 43.2, right = -78.5, top = 44.15),
              ...
              maptype = "terrain") %>%
  ggmap() +
  geom_point(data = group_membership_k4,
             aes(x = long,
             y = lat,
             fill = cluster,
             shape = cluster),
             size = 2,
             color = "black") +
        geom_polygon(data = hulls_k4,
                 mapping = aes_string(
                   x = "long", 
                   y = "lat", 
                   color = as.factor(hulls_k4$cluster), 
                   fill = as.factor(hulls_k4$cluster)),
                 alpha = 0.3) +
  xlab("Longitude") +
  ylab("Latitude") +
  scale_shape_manual(values = c(21,22,23,24))

ggsave(here::here("./Figures_Tables/dapc/k4_map.png"),
         width = 16,
         height = 11,
         units = "cm")

# violin plot of clusters vs urbanization
ggplot(group_membership_k4,
       aes(y = City_dist,
       x = cluster)) +
  geom_violin(aes(y = City_dist,
             x = cluster,
...
ggsave(here::here("./Figures_Tables/dapc/k4_dotplot.png"),
         width = 16,
         height = 11,
         units = "cm")
