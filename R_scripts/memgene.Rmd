...
#   scale_size_continuous(breaks = c(min(mem1$var), max(mem1$var)*50)) +
#   guides(size = FALSE)
...
sf_object <- st_as_sf(spatial_data, coords = c("y", "x"))
...
#### Rescaling eigenvalues from 0-1
```{r}
scaled_eigenvalues <- milkweed_analysis$memSelected %>%
  as.data.frame()

scaled_eigenvalues %<>%
    mutate(across(everything(), ~ (.-min(.))/(max(.)-min(.))))

total_var_scaled <- apply(scaled_eigenvalues, 1, sum) %>%
  as.data.frame() %>%
  dplyr::rename("var" = 1)

# sum all SGS explained by all significant memgene variables
# Combine 'var' from 'total_var' with 'coords'
spatial_data2 <- data.frame(var = total_var_scaled$var, coords)
...
ggsave(last_plot(),
       file =  paste0("./Figures_Tables/memgene/memgene_allMEMs_normalized_", today(), ".png"),
       width = 19,
       height = 12,
       units = "cm")
